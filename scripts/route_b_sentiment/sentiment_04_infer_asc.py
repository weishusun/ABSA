...
Key features
- [Optimized] Uses PyTorch DataLoader for multi-process pre-fetching (fixes IO bottleneck).
- [Optimized] Dynamic Padding: pads to batch longest instead of fixed max_length.
- [Robust] Atomic Write: writes .tmp first then renames to prevent file corruption.
...
- cool-down-time: sleep between batches to prevent thermal throttling
"""
...
from torch.utils.data import DataLoader, Dataset  # [新增] 引入 DataLoader
...
class StringDataset(Dataset):
    """Simple Dataset that holds raw text strings."""
...
class TokenizingCollator:
    """
    Collator that handles tokenization inside the DataLoader worker process.
    Implements Dynamic Padding (padding="longest") for speed.
    """
    ...
    def __call__(self, batch_texts):
        # padding="longest": only pad to the longest seq in THIS batch
        # truncation=True: still respect max_length ceiling
...
def choose_device(no_cuda: bool, no_mps: bool = False) -> torch.device:
    """Select device, priority: CUDA > MPS > CPU"""
...
def load_model_and_tokenizer(
    ...
    # MPS does not support float16, fallback to float32
    if device.type == "mps":
        dtype = torch.float32
...
def infer_probs(
        ...
        max_length: int,
        batch_size: int,
        cool_down_time: float = 0.0,
) -> np.ndarray:
    """
    Revised inference loop using DataLoader for:
    1. Multi-process data preparation (num_workers > 0)
    2. Dynamic Padding (padding='longest')
    """

    # 1. Setup Dataset & Collator
    dataset = StringDataset(texts)
    collator = TokenizingCollator(tokenizer, int(max_length))

    # num_workers=2 is generally safe on Windows/macOS/Linux for this use case.
    # If you encounter 'BrokenPipe' or 'Pickle' errors on Windows, set to 0.
    num_workers = 0
    # Try to detect if we are on a system where multiprocessing might be unstable or slow overhead for small data
    if sys.platform == "win32":
        # Windows spawn overhead can be high, but usually 2 works fine.
        # If dataset is very small, 0 might be faster, but for big data 2 is better.
        pass
    ...
    # MPS does not support AMP, must disable
...
def main() -> int:
    ...
    # cool-down param
    ap.add_argument("--cool-down-time", type=float, default=0.0, help="Sleep seconds between batches to cool down GPU")
    ...
    mps_available = hasattr(torch.backends, 'mps') and torch.backends.mps.is_available() if hasattr(torch.backends,
                                                                                                    'mps') else False
    log_info(
        f"torch={torch.__version__} cuda_available={torch.cuda.is_available()} mps_available={mps_available} device={device} fp16={bool(args.fp16 and device.type == 'cuda')}")
    if device.type == "cuda":
        try:
    ...
    if args.cool_down_time > 0:
        log_info(f"Cool-down enabled: sleep {args.cool_down_time}s between batches.")
    ...
    try:
        for fi, f in enumerate(files):
            ...
            for rg in range(pf.num_row_groups):
                ...
                # infer (Uses DataLoader + Dynamic Padding)
                probs = infer_probs(
                    ...
                    cool_down_time=float(args.cool_down_time),
                )
                ...
                tmp_out_fp = out_fp.with_suffix(".tmp")
                try:
                    pq.write_table(out_tbl, str(tmp_out_fp), compression=compression)
                    if out_fp.exists():
                        out_fp.unlink()
                    tmp_out_fp.rename(out_fp)
                except Exception as e:
                    log_err(f"failed writing parquet: {out_fp} err={e}")
                    if tmp_out_fp.exists():
                        tmp_out_fp.unlink()
                    return 2
        ...
        elapsed = (time.time() - t0) / 60.0
        log_info(
            f"DONE. elapsed={elapsed:.1f}m written_parts={written_parts} skipped_rg={skipped_rg} total_rows={total_rows} out_dir={out_dir}")
        return 0
